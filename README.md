...
- 종목 헤더(현재가·전일 대비)와 Lightweight Charts 기반 캔들/라인 차트, 기간 필터(1주/2주/1개월/전체)
- 차트 포인트 클릭 → **차트 위 3곳**에 결과가 나뉘어 표시됨:
  1. 차트 안, 클릭 지점 바로 옆에 뜨는 작은 팝오버 — "이날 왜 움직였나요?" (공식공시/시장데이터/
     뉴스 근거, `POST /api/analysis/date`)
  2. 차트 카드 하단의 요약 카드 — 선택 날짜/등락률/AI 한줄 요약/핵심 수치 최대 2개/근거 배지
  3. 우측 "오늘의 체크리스트" 패널 — 앞으로 확인할 내용(체크박스) + 더 읽어볼 자료(실제 원문
     링크) + 주의 문구
- 이와 별개로 "주목할 만한 가격변동"(등락락률 상위 카드 스트립) 아래 "관련 자료" 목록과
  SOLAR/Gemini 선택 버튼(`LlmProviderToggle`)이 있음 — 이건 더 예전에 만든
  `POST /api/v1/explanations` 기반 기능으로, 위 3곳과는 별도의 백엔드 호출을 씀(둘 다 같은 클릭
  한 번으로 트리거됨)
- 요인·출처는 **실제 DART 공시 + 네이버 뉴스** 기반 (선택 날짜와 가까운 자료를 우선순위 규칙으로
  선정, 본문 발췌 또는 구조화 필드 사용), 요약·해석은 **실제 SOLAR/Gemini LLM 호출** 결과(키가
  없거나 실패하면 규칙 기반으로 폴백)
- 로딩·성공·오류·빈 자료 상태를 모두 갖춘 팝오버/요약 카드/체크리스트
...
- `SOLAR_API_KEY`, `GEMINI_API_KEY` — **둘 다 실제로 사용됨** (`/api/v1/explanations`용). 프론트
  엔드의 SOLAR/Gemini 선택 버튼(`LlmProviderToggle`, 현재 `MarketEventsPanel` 안에 있음)으로
  고른 쪽의 키를 `llm_service.py`가 사용합니다(기본값 SOLAR). 자동 라우팅/폴백 없음 — 고른
  provider의 키가 없거나 호출이 실패하면 규칙 기반 분석으로만 내려감(CLAUDE.md 9번, 이 구조를
  자동 라우팅으로 되돌리기 전에 꼭 읽을 것). Gemini는 모델명을 `GEMINI_MODEL`(기본
  `gemini-flash-latest` — Google이 관리하는 "항상 최신 무료 Flash 모델" 별칭. `gemini-2.5-flash`
  처럼 날짜 박힌 이름은 신규 키에서 404/유료 전환될 수 있어서 별칭을 기본값으로 씀)로 바꿀 수
  있습니다.
  `LLM_PROVIDER`(기본 `solar`)는 **다른 기능**인 `POST /api/analysis/date`(CLAUDE.md 10번)의
  기본 provider를 고르는 env var — 위 SOLAR/Gemini 선택 버튼과는 무관하고, 지금은 env var로만
  고정(사용자 선택 토글 없음). 이 기능의 Gemini provider는 아직 인터페이스만 있고 실제 호출은
  안 됨.
...
- `NAVER_CLIENT_ID`, `NAVER_CLIENT_SECRET` — **실제로 사용됨**. [Naver Developers](https://developers.naver.com)
  ...
  없어도 스크립트만 못 돌리는 것이고 서비스는 mock으로 폴백합니다. (`.env`에 키를 넣는 것과
  이 스크립트를 실제로 한 번 실행하는 것은 별개입니다 — 실행 전엔 `data/news.json`이 아예
  없어서 공시만 보임.)
...
- **Koyfin/Perplexity 스타일 대시보드 UI**: 상단바 → 종목 선택 + StockHeader(현재가/등락률) →
  캔들/라인 토글 가능한 주가 차트(`ChartTypeToggle`, 클릭 시 선택 정보 표시 + 이날 왜 움직였는지
  팝오버 + 아래에 요약 카드) → "주목할 만한 가격변동"(등락률 상위 카드 + SOLAR/Gemini 선택
  버튼 + 관련 자료) → 우측 "오늘의 체크리스트"(앞으로 확인할 내용 + 더 읽어볼 자료 + 주의 문구).
  자세한 구조·최근 변경 이력은 [CLAUDE.md](CLAUDE.md) 4번/10번 참고
...
- **네이버 뉴스 실데이터**: `data/step5_news.py`를 한 번 실행하면 종목별 실제 기사를
  `data/news.json`에 수집(`.env`에 키만 넣는 것과는 별개 — 스크립트를 실행해야 파일이 생김).
  AI 분석 출처가 이 데이터를 씀
...
- **SOLAR/Gemini 실 LLM 연동 (두 갈래)**:
  - `POST /api/v1/explanations`: 사용자가 버튼(`LlmProviderToggle`)으로 SOLAR/Gemini 직접
    선택(기본 SOLAR), 자동 라우팅/폴백 없음. `[id]` 인용 강제 + 응답 단계 재검증
    (`llm_service._sanitize_factors`). 키가 없거나 호출 실패 시 규칙 기반 분석으로 폴백.
  - `POST /api/analysis/date` (신규, CLAUDE.md 10번): 팝오버/요약 카드/오늘의 체크리스트용.
    env var `LLM_PROVIDER`(기본 solar)로 provider 고정 — 아직 사용자 선택 토글 없음. LLM 출력을
    quick_fact_candidates/allowed_watch_items/available_topics와 대조해서 후보에 없는 값은
    전부 걸러내고(`stock_analysis_service._sanitize_result`), 검증 실패 시 최대 1회 재시도 후
    백엔드 데이터만으로 만든 결정론적 응답으로 폴백.
  - 두 갈래 모두 SOLAR·Gemini 실제 키로 동작 확인됨 — 자세한 내용은
    [docs/project-plan.md](docs/project-plan.md) M3, CLAUDE.md 9번/10번 참고
