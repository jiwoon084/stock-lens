...
The analysis is generated by `app/services/llm_service.py`, which routes each request to
SOLAR-pro2 or Gemini Flash automatically based on how much evidence was retrieved (more source
documents → SOLAR-pro2, few/none → Gemini Flash) — this is not user-selectable. If the routed
provider's API key is unset or the call fails, the other provider is tried once as a
same-request fallback, then a deterministic mock response, so a missing key or provider outage
never causes a 4xx/5xx.
