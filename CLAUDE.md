...
**watch_item 트리거 확장 — 분석만 하고 아직 미구현**: "앞으로 지켜볼 신호"가 잘 안 뜬다는
질문에 `app/rules/watch_item_templates.py`를 까보니, 유일한 공시 트리거 키워드 "공급계약"이
**실제 수집된 `data/disclosures.json` 전체에서 매칭이 0건**이라는 걸 확인함(테스트 픽스처에만
존재하는 가상의 제목). 실제 데이터 빈도 기준으로 자기주식처분결정(4건, major_events.json에
구조화 데이터 있음)·유상증자결정(1건)·잠정실적/영업(잠정)실적(11건, 지금까지 데모에 계속
나온 바로 그 공시)·"실적" 언급 뉴스(12건) 4개 트리거를 추가하자고 제안했으나, **논의만 하고
아직 `watch_item_templates.py`에 반영 안 함** — 다음 세션에서 이어서 할 것.
...
## 13. 일봉 차트에 최근 1~2영업일이 안 뜨는 문제 — KIS로 공식 API의 결제 지연 보완 (2026-07-24)
