...
backend/    FastAPI (routes → schemas/services → core/config 3계층)
  app/api/routes/     health.py, stocks.py, explanations.py
  app/services/       market_data_service.py(공식 KRX API, 없으면 mock), krx_price_client.py
                      ...
                      실제 LLM), explanation_service.py(위 서비스 조합)
...
frontend/   Vite + React + TypeScript (라이트 테마, Koyfin/Perplexity 스타일 대시보드 — 정영준님 작업)
  ...
  src/shared/                         api client(stocks/explanations), types, Card(title+actions)
...
`_providers_for()`, `_SIMPLE_SOURCE_THRESHOLD`)으로 계속 발전시켰는데, 같은 시기 정영준님이
독립적으로 자기 세션에서 다른 방향으로 만들어서 origin에 올림: **"사용자가 SOLAR/Gemini 중
하나를 직접 고르게 해줘, 기본은 SOLAR로"** — 자동 라우팅/폴백이 아니라 사람이 명시적으로
선택하는 구조(`LlmProviderToggle.tsx`, `solar_client.py`/`gemini_client.py`, per-request
`llm_provider` 필드). 두 요구사항이 정면으로 충돌해서 (2026-07-22) 병합 중에 사용자에게 다시
확인했고, **"내 것(자동 난이도 라우팅) 유지"로 최종 확정** — 정영준님의 user-selectable 구조와
관련 파일(`solar_client.py`/`gemini_client.py`/`LlmProviderToggle.tsx`/`test_solar_client.py`/
`test_gemini_client.py`)은 전부 되돌리고 삭제함. 같은 자리에서 "오늘의 체크리스트"도 정영준님
쪽엔 (우리가 지우기 전 버전을 병합으로 물려받아) 남아있었지만, 사용자가 이미 그 세션에서
직접 뺀 결정을 유지하기로 함 — 이것도 다시 삭제. `docs/*.md`/`README.md`의 관련 설명은
현재(자동 라우팅, 체크리스트 없음) 상태에 맞게 고쳐뒀음. 이 항목은 팀원과 소통해서 다음
병합 때 또 되돌이표가 되지 않게 조율하는 게 좋음(7번 참고).

**최종 확정 구조는 위 4번(M3 항목)과 동일** — 근거 자료 개수 기반 자동 라우팅, Groq 없음,
`backend/tests/conftest.py`(LLM_PROVIDER=mock 강제 픽스처) 유지, SOLAR·Gemini 둘 다 실제
키로 검증 완료.
...
1. ~~"오늘의 기사 체크리스트" 카드 추가 여부~~ → 2026-07-20 추가했다가, AI 분석 리포트의
   "주요 요인" 체크리스트와 내용이 겹쳐서 **2026-07-22에 다시 제거함** (백엔드·프론트 코드 전부 삭제).
2. ~~M1(실제 시세)~~ → **완료** (공식 KRX API, 8번 병합 참고). ~~M2(공시·뉴스 실제 연동)~~ →
   **완료**(NAVER_CLIENT_ID/SECRET 발급 완료 + 실행 검증까지 끝남, retrieval_service는 병합으로
   본문 발췌까지 고도화됨).
3. ~~M3(SOLAR/Gemini 실제 LLM)~~ → **완료** — 근거 자료 개수 기반 **자동** 난이도 라우팅으로
   최종 확정(사용자 직접 선택 토글 아님, Groq 없음), 둘 다 실제 키로 동작 확인됨. 정영준님이
   독립적으로 만든 "사용자 직접 선택" 버전은 2026-07-22 병합 때 되돌림(8번 참고).
...
7. **팀 커뮤니케이션 필요**: LLM 구조(자동 라우팅 vs 사용자 선택 토글)와 "오늘의 체크리스트"
   존치 여부를 두고 정영준님과 벌써 두 번 반대 방향으로 각자 작업 후 병합에서 되돌리는 일이
   반복됨(8번 참고). 다음 병합 전에 정영준님과 먼저 합의해두지 않으면 또 충돌할 가능성이 높음.
